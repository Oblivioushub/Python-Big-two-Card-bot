...
card_order_dict = {"3":3, "4":4, "5":5, "6":6, "7":7, "8":8, "9":9, "0":10,"J":11, "Q":12, "K":13, "A":14, "2":15}
def play(hand, is_start_of_round, play_to_beat, round_history, player_no, hand_sizes, scores, round_no):  
  ...
  * `round_no`: An integer between 0 and 9 (inclusive) indicating which round number is currently being played.
  ...
  """
  deck = list(values.keys())
  #card counter
  #example of what round_history = [[(3, ['AD']),(0, ['2S']),(1, [])][(0, ['3C']),(0, ['2S']),(1, [])]]
  ...
  for k, g in groupby(hand, key=lambda x: x[0]):
    if k in mydict:
      ...
      mydict[k]=list(g)
  groups = [v for k, v in mydict.items()]
  hand_tt = []
  ...
  #this is a stupidly long script for checking what play_to_beat is. 
  #full house p1
  def check_full_house(play_to_beat):
    values2 = [i[0] for i in play_to_beat]
  ...
  def check_flush(play_to_beat):
    suits = [i[1] for i in play_to_beat]
    if len(set(suits))==1:
        return True
    else:
        return False
  ...
  def check_straight(play_to_beat): #check straight
    ...
    for v in values2:
      value_counts[v] += 1
    rank_values = [card_order_dict[i] for i in values2]
  ...
  for sublist in fives:
    #check full house
    values1 = [i[0] for i in sublist]
    value_counts = defaultdict(lambda:0)
    for v in values1: 
      value_counts[v]+=1
    if sorted(value_counts.values()) == [2,3]:
      hand_tt.append(sublist)
      fives_fullhouse.append(sublist)
    #check flush
    suits = [h[1] for h in sublist]
    if len(set(suits)) == 1:
      hand_tt.append(sublist)
      fives_flush.append(sublist)
    #check straight
    value_counts = defaultdict(lambda:0)
    for v in values1:
      value_counts[v] += 1
    rank_values = [card_order_dict[i] for i in values1]
    value_range = max(rank_values) - min(rank_values)
    if len(set(value_counts.values())) == 1 and (value_range==4):
      hand_tt.append(sublist)
      fives_straight.append(sublist)
  ...
  hand_tt = [x for x in hand_tt if len(x) < 5]
  ...
  if len(play_to_beat) == 0:
    ...
    if is_start_of_round:
      ...
      if len(hand) < 6: #late game stratergy
        hand_tt.sort(key=len)
        play = hand_tt[0]
        return play
    ...
    #the following program modifys hand in to sublists containing same first character   
    ...
    if len(play_to_beat) > 3: # 5 card plays
      ...
      if check_straight(play_to_beat):#setsup greater for selected group
        greater = [i for i in fives_straight if values.get(i[-1]) > max(ptb)]
        greater.extend(fives_flush)
        greater.extend(fives_fullhouse)
        greater.extend(fives_four)
        greater.extend(fives_straight_flush)

      if check_flush(play_to_beat):#setsup greater for selected group
        greater = [i for i in fives_flush if values.get(i[0]) > max(ptb)]
        greater.extend(fives_fullhouse)
        greater.extend(fives_four)
        greater.extend(fives_straight_flush)

      if check_full_house(play_to_beat):#setsup greater for selected group. This is where it gets stupid complicated. because we have to compare the best card in the the three and avoid the pair
        mydict1={}
        for k, g in groupby(play_to_beat, key=lambda x: x[0]):
          if k in mydict1:
            ...
            mydict1[k]=list(g)
        ptb1 = [v for k, v in mydict1.items()]
          ...
          for sublist in fives_fullhouse:
            ...
            for k, g in groupby(sublist, key=lambda x: x[0]):
              if k in mydict:
                ...
                mydict[k]=list(g)
            f_f1 = [v for k, v in mydict.items()]
      ...
      if check_four_of_a_kind(play_to_beat): #setsup greater for selected group. now that fullhouses are done :)             lets go again :(
        mydict={}
        for k, g in groupby(play_to_beat, key=lambda x: x[0]):
          if k in mydict:
            mydict[k] += g
            ...
            mydict[k]=list(g)
        ptb1 = [v for k, v in mydict.items()]
        ptb1.sort(key=len, reverse = True)
        ptb = []
        if not ptb1:
          greater = []
      ...
      if check_straight_flush(play_to_beat):#setsup greater for selected group. Thank flip there is a easy one in here
        greater = [i for i in fives_straight_flush if values.get(i[-1]) > max(ptb)]
    



    elif len(play_to_beat) == 2: # 2 cards to beat
      pairs = True
      hand_pairs = [] #seprate list to relate 2
      for item in groups:
        if len(item) > 2: #split everything into pairs
          item2 = list(combinations(item, 2))
          item = [list(elem) for elem in item2]
          hand_pairs.extend(item)
        elif len(item) ==2: #add already pairs
          hand_pairs.append(item)
        ptb = []
        for item in play_to_beat:
          ptb.append(values.get(item))
        greater = [i for i in hand_pairs if values.get(i[-1]) > max(ptb)]
        
    elif len(play_to_beat) == 3: # 3 cards to beat
      threes = True
      hand_threes = [] #seprate list to relate 2
      for item in groups:
        if len(item) > 3: #split everything into pairs
          item2 = list(combinations(item, 3))
          item = [list(elem) for elem in item2]
          hand_threes.extend(item)
        elif len(item) ==3: #add already threes
          hand_threes.append(item)
        ptb = []
        for item in play_to_beat:
          ptb.append(values.get(item))
        greater = [i for i in hand_threes if values.get(i[-1]) > max(ptb)]
        
    elif len(play_to_beat) == 1: # 1 card to beat
      singles = True
      greater = [i for i in hand if values.get(i) > values.get(play_to_beat[-1])] #list of cards in hand greater then card just played
      greater = [[i] for i in greater]
    ...
    if not greater:#if bot cant beat
      ...
      if singles:
        ...
        if all(elem in beating_cards  for elem in played_cards_values) and (len(hand) < 6): # I will get control of the board with this card
          play = greater[-1]
          return play
      elif pairs:
        mydict={}
        for k, g in groupby(remaining_cards, key=lambda x: x[0]):
          if k in mydict:
            ...
            mydict[k]=list(g)
        remaining_cards1 = [v for k, v in mydict.items()]
...
if __name__ == '__main__':
  ...
  TESTS = [  # [ expected return value, inputs ]
    ...
    [['3D', '3C'], [['3D', '3C', '4H', '7D', '8D', '8H', '0D', '0C', 'JH', 'QC', 'QS', 'KH', 'AS'], True, [], [[]], 0, [13, 13, 13, 13], [0, 0, 0, 0], 0]],
    [['3D', '3C', '3H', '4D', '4D'], [['3D', '3C', '3H', '4D', '4D', '8H', '0D', '0C', 'JH', 'QC', 'QS', 'KH', 'AS'], True, [], [[]], 0, [13, 13, 13, 13], [0, 0, 0, 0], 0]],
    [['3D', '4C', '5H', '6D', '7D'], [['3D', '4C', '5H', '6D', '7D', '8H', '0D', '0C', 'JH', 'QC', 'QS', 'KH', 'AS'], True, [], [[]], 0, [13, 13, 13, 13], [0, 0, 0, 0], 0]],
    [[], [['8C', '9D'], False, ['7H', '8S', '9C', '0S', 'JC'], [[[3, ['3D', '3H', '3S', '6C', '6S']], [0, []], [1, []], [2, []]], [[3, ['5S']], [0, ['JH']], [1, ['KC']], [2, []], [3, ['KS']], [0, ['AC']], [1, []], [2, []], [3, ['2C']], [0, []], [1, []], [2, []]], [[3, ['8D']], [0, ['2S']], [1, []], [2, []], [3, []]], [[0, ['4H', '8H', '9H', 'QH', 'KH']], [1, []], [2, []], [3, []]], [[0, ['5D']], [1, ['6H']], [2, []], [3, ['9S']], [0, ['QD']], [1, ['AH']], [2, []], [3, ['2H']], [0, []], [1, []], [2, []]], [[3, ['0D']], [0, ['KD']], [1, ['AS']], [2, []], [3, []], [0, []]], [[1, ['7H', '8S', '9C', '0S', 'JC']], [2, []], [3, []]]], 0, [2, 4, 13, 1], [0, 0, 0, 0], 0]],
    [['KD'], [['6D', '6S', '7H', '8S', '9C', '9S', '0D', 'JC', 'KD', 'KC', 'AD', 'AH'], False, ['QC'], [[[3, ['3D']], [0, ['3H']], [1, ['5C']], [2, ['JS']], [3, ['QC']]]], 0, [12, 12, 12, 11], [11, -9, 9, -11], 2]],
    [[], [['4D', '5C', '5S', '7D', '8D', '0D', '0S', 'JH', 'QD', 'KD', 'KS', '2H', '2S'], False, ['5H', '6H', '7H', '8H', '9H'], [[[3, ['3D', '3C', '3H']], [0, ['6D', '6C', '6S']], [1, []], [2, []], [3, []]], [[0, ['9D', '9C', '9S', 'QC', 'QS']], [1, ['5H', '6H', '7H', '8H', '9H']]]], 2, [5, 8, 13, 10], [26, -34, 61, -53], 8]],
    [['9C', '9H', 'JD', 'JC', 'JS'], [['4S', '5S', '6D', '8S', '9C', '9H', '9S', 'JD', 'JC', 'JS', 'KS', 'AD', '2C'], False, ['0D', '0H', '0S', '3C', '3D'], [[[3, ['0D', '0H', '0S', '3C', '3D']]]], 0, [13, 13, 13, 8], [-13, -117, 47, 83], 3]]
  ]
